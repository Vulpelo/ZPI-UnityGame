...
public class EnchancedFirstPersonController : MonoBehaviour
{
    ...
    [SerializeField]
    private float m_FallingThreshold;
    ...
    [SerializeField]
    private float m_AntiBumpFactor;
    ...
    [SerializeField]
    private int m_AntiBunnyHopFactor;
    ...
    [Tooltip("Health class allowing character taking damage.")]
    [SerializeField]
    ...
    {
        if (m_Controller.velocity.sqrMagnitude > 0 && (m_MoveDirection.x != 0 || m_MoveDirection.y != 0))
        {
            m_StepCycle += (m_Controller.velocity.magnitude + (speed * (m_IsWalking ? 1f : m_RunstepLenghten))) *
                         Time.fixedDeltaTime;
        }
    ...
    {
        health.TakeDamage(System.Convert.ToInt32(fallDistance*2));
    }
}